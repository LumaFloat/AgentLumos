...
通常只需要先配置键盘上可见的 Lock 指示灯，再安装对应 agent 的 hook。之后 Codex 或 Claude Code 运行时会通过 hooks 自动触发 `working`、`blocked`、`success`、`error` 等状态，不需要日常手动执行状态命令。
...
AgentLumos 通过 visual profile 为一灯、两灯、三灯布局显式选择动画和速度。profile 会显式覆盖 `working` 这类基础状态，也会覆盖 `working.command`、`error.critical` 这类内置 state kind。三灯布局保持完整默认动画；两灯布局使用左右移动和双灯脉冲；一灯布局使用不同节奏，让 `working`、`blocked`、`success` 和 `error` 仍然容易区分。
...
- `working` 会在每次匹配的 hook 到来时续租，过期后不会回放。
